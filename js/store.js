...
function reshowingList() {
  ...
  if (schedules === undefined) {
    ...
    const newSchedule = schedules.filter(
      (schedule) => schedule.date == keyValue
    );
...
const openModal = (e) => {
  ...
  amendId = e.target.parentElement.id;
...
function deleteSchedule(e) {
  ...
  const result = schedules.filter(
    (schedule) => schedule.id !== Number(thisSchedule.id)
  );
  schedules = result;
  saveSchedule();
}
  ...
  for (let i = 0; i < schedules.length; i++) {
    if (schedules[i].id == amendId) {
      ...
      schedules.map((schedule) =>
        schedules[i].id === amendId ? `${schedules[i].text}` : schedule
      );
